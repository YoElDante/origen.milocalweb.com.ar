...
<section id="productos" class="section section-productos" aria-label="Productos destacados">
    <div class="section-container">
        <h2 class="section-title">Ropa Deportiva</h2>
        <p class="section-subtitle">Running, trekking y bikes en Río Tercero — Calzas, shorts, tops y accesorios</p>

        <div class="productos-grid">
            <?php foreach ($productos as $i => $p): ?>
            <?php
                $prod_name = htmlspecialchars_decode($p['nombre'] ?? '');
                $prod_msg  = urlencode('Hola! Vi ' . $prod_name . ' en tu web y quisiera más información');
                $prod_wa   = $wa_link . '?text=' . $prod_msg;
                $img_file  = $p['imagen'] ?? '/assets/img/cliente/productos/producto-' . ($i + 1) . '.jpg';
            ?>
            <article class="producto-card">
                <div class="producto-img-wrapper">
                    <img src="<?= htmlspecialchars($img_file) ?>"
                         alt="<?= htmlspecialchars($p['nombre'] ?? 'Producto ' . ($i + 1)) ?> — Origen Run & Bike en Río Tercero"
                         class="producto-img"
                         width="600"
                         height="450"
                         loading="lazy">
